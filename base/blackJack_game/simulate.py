...
class Card:
    '''
       ...
       solution:对于主要是用来当成简单的数据结构的类而言，你可以通过给类添加 __slots__ 属性来极大的减少实例所占的内存。比如：下方声明
    ...
    def __repr__(self):
        '''
            ...
            solutions:要改变一个实例的字符串表示，可重新定义它的 __str__() 和 __repr__() 方法。例如：
            :return:
        '''
    ...
    def shuffle(self):
        for _ in range(self.num):
            for index in range(len(self.cards)):
                i = random.randint(0,15)  # randint(n,m,[x,y])  n行 m列 取值范围 左闭右开
...
class User:
    ...
    def speak(self,content="",end_char="\n"):
        print("");
        print(
            COLOR.get(self.color)+self.prompt + COLOR.get("end")+content,end=end_char
        )
...
class Dealer(User):
    def __int__(self,name,amount):
        super().__init__(name=name, role="Dealer", color="PURPLE") # 确保父类被正确的初始化了
        self.trigger = 0
...
class Recorder:
    ...
    def update(self,winner,chips,player_points,dealer_point):
        self.rounds += 1
        ...
        self.player_point = player_points
        self.dealer_point = dealer_point

    # 分数
    def record(self,winner,chips,player_points,dealer_point):
        self.update(winner,chips,player_points,dealer_point)
        Row = namedtuple(
